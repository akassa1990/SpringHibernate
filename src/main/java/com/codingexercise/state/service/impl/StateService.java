	...
	public States getAlabamaAndGeorgia() {
		...
		while (hitsElements.hasNext()) {
			...
			if (myState.path("name").asText().equalsIgnoreCase("Alabama")
				...
				State state = new State(myState.path("id").asInt(), 
										myState.path("country").asText(),
										myState.path("name").asText(), 
										myState.path("abbr").asText(), 
										myState.path("area").asText(),
										myState.path("largest_city").asText(), 
	...
	public State getStatesByName(String state) {
		...
		while (hitsElements.hasNext()) {
			...
			if (myState.path("name").asText().equalsIgnoreCase(state)) {
				 stateObject = new State(myState.path("id").asInt(), 
							myState.path("country").asText(),
							myState.path("name").asText(), 
							myState.path("abbr").asText(), 
							myState.path("area").asText(),
